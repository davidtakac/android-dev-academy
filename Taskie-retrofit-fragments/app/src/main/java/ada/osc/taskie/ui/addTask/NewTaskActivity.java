...
public class NewTaskActivity extends AppCompatActivity implements NewTaskContract.View {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(isCalledByEditIntent()){
            String clickedTaskId = getIntent().getStringExtra(TasksActivity.EXTRA_TASKTOEDIT);
            presenter.setEntriesToValuesOf(clickedTaskId);
        }
    ...
    public void saveTask() {
        ...
        if(isCalledByEditIntent()) {
            String clickedTaskId = getIntent().getStringExtra(TasksActivity.EXTRA_TASKTOEDIT);
            presenter.updateTaskWithValues(clickedTaskId, title, description, priority);
        } else {
            Task newTask = new Task(title, description, priority.ordinal() + 1);
    ...
    public void setContent(String content) {
        mDescriptionEntry.setText(content);
    }

    @Override
    public void setPriority(int priority) {
        mPriorityEntry.setSelection(priority);
    }

    public boolean isCalledByEditIntent(){
        return getIntent().hasExtra(TasksActivity.EXTRA_TASKTOEDIT);
    }
}
